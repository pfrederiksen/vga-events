## Dependency Update Report

Generated: Sun Feb 22 09:19:33 UTC 2026

## Available Dependency Updates

- **github.com/cpuguy83/go-md2man/v2**: v2.0.6 → v2.0.7
- **github.com/google/go-cmp**: v0.6.0 → v0.7.0
- **github.com/spf13/pflag**: v1.0.9 → v1.0.10
- **github.com/yuin/goldmark**: v1.4.13 → v1.7.16
- **golang.org/x/crypto**: v0.47.0 → v0.48.0
- **golang.org/x/mod**: v0.17.0 → v0.33.0
- **golang.org/x/net**: v0.48.0 → v0.50.0
- **golang.org/x/sync**: v0.10.0 → v0.19.0
- **golang.org/x/sys**: v0.40.0 → v0.41.0
- **golang.org/x/telemetry**: v0.0.0-20240228155512-f48c80bd79b2 → v0.0.0-20260213145524-e0ab670178e1
- **golang.org/x/term**: v0.39.0 → v0.40.0
- **golang.org/x/text**: v0.33.0 → v0.34.0
- **golang.org/x/tools**: v0.21.1-0.20240508182429-e35e4ccd0d2d → v0.42.0
- **golang.org/x/xerrors**: v0.0.0-20190717185122-a985d3407aa7 → v0.0.0-20240903120638-7835f813f4da
- **gopkg.in/check.v1**: v0.0.0-20161208181325-20d25e280405 → v1.0.0-20201130134442-10cb98267c6c

## Security Scan

✅ No vulnerabilities found in updated dependencies

## Test Results

✅ All tests pass with updated dependencies
